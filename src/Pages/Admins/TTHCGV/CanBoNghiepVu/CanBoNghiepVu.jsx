...
import { TEMPLATE_SUBJECT_RECEIVED_EMAIL, sendEmailTTHCGiangVien } from "../../../../Services/Utils/emailUtils";
		...
		if (itemYeuCau?.MC_TTHC_GV_GuiYeuCau_TrangThai_ID == 0 || !itemYeuCau?.MC_TTHC_GV_GuiYeuCau_TrangThai_ID) {
			Swal.fire({
				...
				if (result.isConfirmed) {
					...
					const resListTPHSDeNghiYeuCau = await getThanhPhanHoSoByIdTTHCGV(itemYeuCau?.MC_TTHC_GV_GuiYeuCau_YeuCau_ID);
					...
					if (resNewTrangThaiID.status === 200) {
						...
						if (dataTrangThaiIDNew) {
							...
							if (resPutHoSoThuTuc.status === 200) {
								const res = await sendEmailTTHCGiangVien(TEMPLATE_SUBJECT_RECEIVED_EMAIL, itemYeuCau, dataCBGV, listTPHSDeNghiYeuCau);
								setLoading(false);
								fetchData();
