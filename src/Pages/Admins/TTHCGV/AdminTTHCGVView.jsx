    ...
    const handleOnSubmitForm = async (e) => {
        ...
        const dataThongTinHoSo = {
            ...
            MC_TTHC_GV_TepThuTuc_DataFileFile:
                dataFilesTepThuTuc?.MC_TTHC_GV_TepThuTuc_DataFileFile?.split(
                    ',',
                )[1],
        }
        ...
        if (quyTrinh.length > 0) {
            for (let i = 0; i < quyTrinh.length; i++) {
                if (
                    ...
                    quyTrinh[
                        i
                    ]?.MC_TTHC_GV_TrinhTuThucHien_TenCongViec.trim() == ''
                ) {
                    Swal.fire({
                        icon: 'warning',
                        title: 'Lỗi',
        ...
        if (phanQuyen.length < 1) {
            ...
            setTrangThaiActive(false)
            return toast.error(
                'Vui lòng thiết lập phân quyền cho người thực hiện!',
            )
        }
        ...
        if (trangThai.length < 1) {
            ...
            setTrangThaiActive(true)
            return toast.error(
                'Vui lòng thiết lập trạng thái cho hồ sơ thủ tục!',
            )
        } else {
        ...
        try {
            ...
            if (resultPostThongTinTTHC.status === 200) {
                ...
                if (dataPostThongTinHoSo.message === 'Bản ghi bị trùng.') {
                    ...
                    const dataTTHCGVGetID =
                        await getThuTucHanhChinhByMaThuTuc(maThuTuc)
            ...
            try {
                if (
                    ...
                    const dataPostThanhPhanHoSo =
                        await resultPostThanhPhanHoSo.data
                    const dataPostTrinhTuThucHien =
                        await resultPostTrinhTuThucHien.data
