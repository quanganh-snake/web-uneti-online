...
import { getDetailEditStatusYeuCau, getHoSoGuiYeuCauById, getTrangThaiXuLyYeuCauById } from "../../../../Apis/ThuTucHanhChinhGiangVien/apiThuTucHanhChinhGiangVien";
...
function TheoDoiDeNghiTTHCGVChiTiet() {
	...
	const handleActive = async (currentDataStatus, index) => {
		...
		getDetailEditStatusYeuCau(id, currentDataStatus?.MC_TTHC_GV_TrangThai_TenTrangThai)
				...
				<>
					...
					<div className="mb-4 border p-2 bg-white">
						...
						<div className="w-full flex flex-col lg:flex-row justify-between gap-10">
							<table className={clsx("border", activeDetail ? "w-full max-h-[200px]" : "w-1/2")}>
								...
								<tbody>
									{listTrangThai?.length > 0 &&
										listTrangThai?.map((iTrangThai, index) => {
											return (
												<tr key={index} className={clsx("border-b ", currentStatusByProcedure == iTrangThai?.MC_TTHC_GV_TrangThai_ID ? "bg-slate-300" : "")}>
													<td className="text-center border-r">{iTrangThai?.MC_TTHC_GV_TrangThai_STT}</td>
													<td className="text-center border-r">{iTrangThai?.MC_TTHC_GV_TrangThai_TenTrangThai}</td>
													<td className="text-center border-r">
														{iTrangThai?.MC_TTHC_GV_GuiYeuCau_DateEditor ? moment(iTrangThai?.MC_TTHC_GV_GuiYeuCau_DateEditor).format("DD/MM/YYYY HH:mm:ss") : null}
													</td>
							...
							{activeDetail ? (
								<div className="flex flex-col gap-4 w-full">
									<h4 className="font-semibold">Chi tiết thực hiện của công việc: {tenTrangThai.toUpperCase()}</h4>
										...
										<table className="w-full">
											<thead className="bg-sky-800 text-white sticky">
												<tr>
													...
													<th className="px-2 whitespace-nowrap border">Người xử lý</th>
													<th className="px-2 whitespace-nowrap border">Ngày hẹn trả</th>
													<th className="px-2 whitespace-nowrap border">Nơi trả kết quả</th>
													<th className="px-2 whitespace-nowrap border">Ngày xử lý</th>
												</tr>
											</thead>
											<tbody>
												{detailEditStatus.map((iDetail, index) => {
													return (
														<tr className="border" key={index}>
															...
															<td className="px-2 py-1 text-center border-r">{iDetail?.HoTen}</td>
															<td className="px-2 py-1 text-center border-r">
																{iDetail?.MC_TTHC_GV_GuiYeuCau_NgayHenTra && moment(iDetail?.MC_TTHC_GV_GuiYeuCau_NgayHenTra).format("DD/MM/YYYY HH:mm:ss")}
															</td>
															<td className="px-2 py-1 text-center border-r">{iDetail?.MC_TTHC_GV_GuiYeuCau_NoiTraKetQua}</td>
															<td className="px-2 py-1 text-center border-r">
																{iDetail?.MC_TTHC_GV_GuiYeuCau_DateEditor ? moment(iDetail?.MC_TTHC_GV_GuiYeuCau_DateEditor).format("DD/MM/YYYY HH:mm:ss") : null}
															</td>
														</tr>
													);
												})}
