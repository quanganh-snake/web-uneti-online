			...
			{loading ? (
				...
				<div className="flex flex-col md:flex-row gap-2">
					...
					<div className="grow bg-white w-full p-4">
						...
						<form onSubmit={handleSubmitForm} className="grid grid-cols-2 gap-4 mt-5">
							<h2 className="text-lg mb-4 col-span-2">
								<span className="font-semibold">Tên thủ tục: </span>
								<span className="uppercase font-bold">{dataChiTietThuTuc?.ThongTinHoSo?.MC_TTHC_GV_TenThuTuc}</span>
							...
							<div className="flex flex-col form-group mb-4 col-span-1">
								<label htmlFor="MC_TTHC_GV_GuiYeuCau_NhanSuGui_SDT" className="font-semibold mb-2">
									Số điện thoại liên hệ <span className="font-bold text-red-500">*</span>
								</label>
								<input
									type="tel"
									className="border border-slate-300 px-2 py-1 rounded-xl focus:outline-1 focus:outline-slate-300"
							...
							<div className="flex flex-col form-group mb-4 col-span-2">
								<label htmlFor="price" className="font-semibold mb-2">
									Danh sách giấy tờ kèm theo <span className="font-bold text-red-500">*</span>
								</label>
								{dataChiTietThuTuc?.ThanhPhanHoSo?.length ? (
															...
															{iThanhPhanHoSo?.MC_TTHC_GV_ThanhPhanHoSo_TenFile ? (
																<p className="w-[320px] text-sm text-[#336699]">
																	...
																	<span
																		...
																		onClick={() => {
																			const base64StringWithoutPrefix = convertBufferToBase64(iThanhPhanHoSo?.MC_TTHC_GV_ThanhPhanHoSo_DataFile?.data);
																			handlePreviewFileBase64(iThanhPhanHoSo?.MC_TTHC_GV_ThanhPhanHoSo_TenFile, base64StringWithoutPrefix);
																		}}
																	>
																		{iThanhPhanHoSo?.MC_TTHC_GV_ThanhPhanHoSo_TenFile}
																	</span>
									...
									<p className="italic text-red-700 border p-2 font-medium rounded-md">Không có thành phần hồ sơ kèm theo!</p>
								)}
							</div>
							<div className="flex flex-row gap-4 col-span-2">
								<button type="submit" className="flex flex-row gap-2 items-center cursor-pointer hover:opacity-70 bg-green-700 text-white border border-slate-100 px-2 py-1 rounded-xl">
									<BsSend />
									<span>Nộp hồ sơ</span>
								</button>
								<button
