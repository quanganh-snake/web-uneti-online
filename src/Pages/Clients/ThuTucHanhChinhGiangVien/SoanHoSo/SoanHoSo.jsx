    ...
    const handleChangeInputFileTPHS = async (idTPHS, e) => {
        ...
        if (file) {
            ...
            if (existFileIndex !== -1) {
                ...
                listThanhPhanHoSoFiles[existFileIndex] =
                    newItemThanhPhanHoSoFile
            } else {
                ...
                if (resultKiemTraHoSoThuTucTrung.status === 200) {
                    ...
                    if (resultCountKiemTra > 0) {
                        ...
                        const resultPostTTHCYeuCau =
                            await postThuTucHanhChinhGuiYeuCau(
                                newDataHoSoYeuCau,
                            )

                        if (resultPostTTHCYeuCau.status === 200) {
                            const getIDGuiYeuCau =
                                await getGuiYeuCauHoSoThuTucKiemTraTrung(
                                    dataCBGV.MaNhanSu,
                                    newDataHoSoYeuCau?.MC_TTHC_GV_GuiYeuCau_YeuCau_ID,
                                    newDataHoSoYeuCau?.MC_TTHC_GV_GuiYeuCau_TrangThai_ID,
                                )
                            const dataIDGuiYeuCau =
                                await getIDGuiYeuCau.data?.body[0]
                            idGuiYeuCau =
                                await dataIDGuiYeuCau?.MC_TTHC_GV_GuiYeuCau_ID
                            let khoaGiangVien =
                                await dataIDGuiYeuCau?.MC_TTHC_GV_GuiYeuCau_NhanSuGui_Khoa
                            // UI-POST: Thanh Phan Ho So
                            for (
                                let i = 0;
                                i < listThanhPhanHoSoFiles.length;
                                i++
                            ) {
                            ...
                            const resultPostTPHSGuiYeuCau =
                                await postThanhPhanHoSoGuiYeuCau(
                                    listThanhPhanHoSoFiles,
                                )
