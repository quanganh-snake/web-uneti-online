...
function DangKyLopChatLuong() {
    ...
    useEffect(() => {
        ...
        if (hocKy !== '') {
            getAllHocPhanDKLopChatLuong(hocKy, dataSV.MaSinhVien).then(
                (res) => {
    ...
    const handleSubmitData = async (e) => {
        ...
        let dataHocPhan = makePostDataSv(
            makeDataSv(dataSV, MC_DT_DKHocChatLuong_PREFIX),
            {
                ...
                GhiChu: transformSubmitValue(
                    hocPhan.MC_DT_DKHocChatLuong_GhiChu,
                ),
                YeuCau_LoaiHocTap: '0',
                YeuCau_LyDo: lyDo,
                YeuCau_LyDo_LyDoKhac_LyDoChiTiet:
                    transformSubmitValue(lyDoKhac),
            },
    ...
    const handlePostData = async (dataHocPhan) => {
        try {
            ...
            //Kiểm tra trùng
            const checkTrungDKLopChatLuong =
                await getKiemTraTrungDKLopChatLuong(
