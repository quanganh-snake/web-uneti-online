...
function KetQuaHocTapView(props){
    ...
    const {home, breadcrumbs, tenDot, setTenDot, lyDo, setLyDo, listHocKy, listLyDo, listHocPhan, listLyDoDTK, listLyDoDT, lyDoChiTiet, setLyDoChiTiet, diemSua, setDiemSua, handleRowSelection, handleSubmitData} = props
			...
			<div className="p-4 flex flex-col gap-4">
				...
				<div className="border-[#336699] border border-solid mt-5 rounded-md">
                    <form className="py-8 flex flex-col justify-center items-center gap-4">
                        <h2 className="text-center uppercase text-2xl font-bold text-sky-800 mb-6">TIẾP NHẬN KẾT QUẢ HỌC TẬP</h2>
                    ...
                    { (tenDot !== '' && lyDo !== '') && <DanhSachHocPhan tenDot={tenDot} lyDo={lyDo} listHocPhan={listHocPhan} handleRowSelection={handleRowSelection} />}
                    { (tenDot !== '' && lyDo !== '' && lyDo !== 'Xem kết quả học tập') && 
                    <FormYeuCauSuaDiem 
                        lyDo={lyDo} 
