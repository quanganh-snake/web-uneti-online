...
function GiayGioiThieuView(props) {
  const { home, breadcrumbs, handleDownloadFile, listMauDeNghi } = props;
      ...
      <div className="p-4 flex flex-col">
        ...
        <div className="bg-yellow-100 w-full rounded-md mt-4 p-3 flex flex-col justify-center items-center min-h-[500px] text-[#856404]">
          ...
          <ul className="w-full ml-10">
            {listMauDeNghi.map((MauDeNghi, index) => (
              <li key={index} className="w-full font-bold mb-4 text-sm">
                1.{index + 1}
                <span
                  className="cursor-pointer text-[#245D7C] px-2 hover:text-[#0056b3] duration-200"
                  onClick={(e) => handleDownloadFile(e, MauDeNghi)}
                >
                  {MauDeNghi}
                </span>
              </li>
...
GiayGioiThieuView.propTypes = {
  home: PropTypes.object,
  breadcrumbs: PropTypes.array,
  handleDownloadFile: PropTypes.func,
  listMauDeNghi: PropTypes.array,
};

export default GiayGioiThieuView;
